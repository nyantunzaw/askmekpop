...
function addLights() {
    ...
    var greenPoint = new THREE.PointLight(0xffffff, 1, 150);
    greenPoint.position.set( -70, 50, 70 );
	...
	spL2 = new THREE.SpotLight(0xffffff, 1, 300,20,1000);
	spL2.position.set(30,150,0);
	spL2.target.position.set(0,0,0);
	scene.add(spL2);
	scene.add(spL2.target);
    
    var spotTarget = new THREE.Object3D();
    spotTarget.position.set(0, 0, 0);
    spotLight.target = bluePoint;
...
function addSceneElements() {
	...
	//Added by myself
	var logo = new THREE.MeshPhongMaterial( { map: THREE.ImageUtils.loadTexture('images/nteezy.png') } );
	
		
	var middleWall = new THREE.Mesh(cube, logo);
	middleWall.rotation.x = Math.PI/180 * 90;
    middleWall.position.set(0,50,-20);
...
function animate() {
	...
	sphere.position.x = Math.sin(counter) * 100;
